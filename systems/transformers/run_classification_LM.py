...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #pooled_output = outputs[3] ######## CLS embeddinak #############
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""): 
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # pooled_output=outputs[3] ####### CLS embeddingak ?????????????????????????????????????????????????
...
def main():
            ...
            with open(os.path.join(args.data_dir, "dev.tsv"), "r") as f: 
                ...
                for line in lines_prestatuta: 
                    if predictions[example_id]:
                        text_ex = ""
                        if len(line.split()) > 1:
                            text_ex = line.split('\t')[2] # textuaren zutabea 2
    ...
    if args.do_predict and args.local_rank in [-1, 0]: ###### TEST #######
        ...
        with open(output_test_predictions_file, "w") as writer:
            writer.write("TWEET_ID\tuser_id\tTWEET\tLABEL\tpredicted\tAGAINST\tFAVOR\tNEUTRAL\n") 					 	
            with open(os.path.join(args.data_dir, "test.tsv"), "r") as f:
                ...
                for line in lines_prestatuta:
                    if predictions[example_id]:
                        text_ex = ""
                        if len(line.split()) > 1:
                            text_ex = line.split('\t')[2] # textuaren zutabea 3
                            if args.get_all_preds:
                                #output_line = predictions[example_id] + "\t" + text_ex + "\n"
